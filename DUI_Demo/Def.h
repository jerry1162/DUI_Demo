...
#define CM_MOUSEGETIN (WM_USER + 2) //表示鼠标进入控件，注意，此消息与WM_MOUSEMOVE不同，只在鼠标进入控件时发送，鼠标在控件上移动时不会发送此消息
#define CM_STATECHANGED (WM_USER + 3)//当控件状态发生变化时，收到此消息，wParam是指控件ID，lParam为上一个状态 返回0表示不需要在ChangeState里调用Update.
...
#define CM_SHOW (WM_USER + 5)//组件显示或隐藏 wParam是指控件ID，lParam表示显示状态
	#define CS_SHOW (1)
	#define CS_HIDE (0)
#define CM_SETAUTOUPDATE (WM_USER + 6)//用于设置自动刷新属性 wParam是指控件ID，lParam表示属性值

#define WM_WNDINITED (WM_USER + 7) //用于处理窗口初始化完后的一些工作
